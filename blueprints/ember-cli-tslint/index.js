...
module.exports = {
  ...
  afterInstall() {
    ...
    /**
   * Find ESLint configuration files and offer to remove them
   *
   * @return {RSVP.Promise}
   */
  _removeESLintConfig() {
    ...
    return this._findESLintConfigFiles()
      .then(function(files) {
        ...
        ui.writeLine('\nThe following ESLint config files were found:');
   ...
   */
  _findESLintConfigFiles() {
    var projectRoot = this.project.root;
    var ui = this.ui;

    ui.startProgress('Searching for ESLint config files');
    return new RSVP.Promise(function(resolve) {
      var files = walkSync(projectRoot, {
        globs: ['**/.eslintrc.js', '**/.eslintrc', '.eslintrc.yaml', '.eslintrc.json', '.eslintrc.yml'],
        ignore: [
          '**/bower_components',
          '**/dist',
          '**/node_modules',
